...
export class RunnableMap<RunInput = any, RunOutput = any> extends Runnable<RunInput[], RunOutput[]> {
  ...
  lc_namespace = ['custom', 'runnable', 'map'];
  ...
  async invoke(input: RunInput[], config?: MapConfig): Promise<RunOutput[]> {
    ...
    try {
      ...
      if (config?.callbacks && typeof (config.callbacks as any).onChainEnd === 'function') {
        const duration = Date.now() - startTime;
        await (config.callbacks as any).onChainEnd({
      ...
      if (config?.callbacks && typeof (config.callbacks as any).onChainError === 'function') {
        await (config.callbacks as any).onChainError({
          chainId: 'runnable_map',
          inputs: { input, count: input.length },
  ...
  /**
   * Custom stream mapping results (does not override base)
   */
  async *streamMap(input: RunInput[], config?: MapConfig): AsyncGenerator<RunOutput[]> {
    const concurrency = config?.concurrency || this.concurrencyLimit;
  ...
  static from<Input, Output>(
    ...
    // Ensure type safety: always return RunnableMap<Input, Output>
    return new RunnableMap(lambdaRunnable as RunnableInterface<Input, Output>, options);
  }
  ...
  withReduce<ReduceOutput>(
    ...
    // Ensure type safety: flatten double promise
    ...
    }) as unknown as Runnable<RunInput[], ReduceOutput>;
  }

  /**
   * Create a map that retries failed items (custom, does not override base)
   */
  withCustomRetry(maxRetries: number = 3, baseDelay: number = 1000): RunnableMap<RunInput, RunOutput> {
    const retryRunnable = RunnableLambda.from(async (input: RunInput) => {
      let attempt = 0;
      while (attempt <= maxRetries) {
        try {
          const result = await this.mapRunnable.invoke(input);
          return result;
        } catch (error) {
          attempt++;
    ...
    // Ensure type safety: always return RunnableMap<RunInput, RunOutput>
    return new RunnableMap(retryRunnable as unknown as RunnableInterface<RunInput, RunOutput>, {
      concurrency: this.concurrencyLimit,
      batchSize: this.batchSize,
      preserveOrder: this.preserveOrder
    });
  ...
  withRateLimit(requestsPerSecond: number): RunnableMap<RunInput, RunOutput> {
    ...
    // Ensure type safety: always return RunnableMap<RunInput, RunOutput>
    return new RunnableMap(rateLimitedRunnable as unknown as RunnableInterface<RunInput, RunOutput>, {
